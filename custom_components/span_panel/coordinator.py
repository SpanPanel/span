...
class SpanPanelCoordinator(DataUpdateCoordinator[SpanPanel]):
    ...
    def __init__(
        ...
        # Detect Gen3 push-streaming panels (for logging only — both use polling).
        is_push_streaming = PanelCapability.PUSH_STREAMING in span_panel.api.capabilities

        # Both Gen2 and Gen3 use a polling timer to push data to HA.
        # For Gen3, the gRPC stream keeps the client buffer fresh every ~1s
        # in the background; the coordinator reads it at the scan interval.
        # This prevents overwhelming HA's recorder with 1/s state writes.
        ...
        mode_label = "Gen3 push-buffered" if is_push_streaming else "Gen2 polling"
        _LOGGER.info(
            "Span Panel coordinator: %s mode, update interval %s seconds",
            mode_label,
            scan_interval_seconds,
        )
        update_interval: timedelta | None = timedelta(seconds=scan_interval_seconds)
