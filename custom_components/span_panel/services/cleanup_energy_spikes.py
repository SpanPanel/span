                ...
                if time_diff <= 120:
                    ...
                    if i > 0 and current_sum is not None:
                        ...
                        if prev_sum is not None and prev_sum > current_sum:
                            ...
                            # Call the recorder service to adjust
                            # Format: statistic_id, start_time (ISO string), adjustment (float)
                            service_data = {
                                "statistic_id": entity_id,
                                "start_time": entry_time.strftime("%Y-%m-%d %H:%M:%S"),
                                "adjustment": float(adjustment),
                            }
                            _LOGGER.warning(
                                "ADJUST: Calling recorder.adjust_sum_statistics: "
                                "entity=%s, time=%s, adjustment=+%.2f",
                                entity_id,
                            ...
                            try:
                                ...
                                sensors_adjusted += 1
                                _LOGGER.warning("ADJUST: Successfully adjusted %s", entity_id)
                            except Exception as e:
