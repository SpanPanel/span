        ...
        while iteration < max_iterations:
            ...
            for i in range(1, len(sensor_stats)):
                ...
                if prev_sum > current_sum:
                    ...
                    # Log why missing_energy might be 0
                    if missing_energy == 0:
                        if i + 1 >= len(sensor_stats):
                            _LOGGER.debug(
                        ...
                        elif i + 1 < len(sensor_stats):
                            next_entry_check = sensor_stats[i + 1]
                            next_sum_check = next_entry_check.get("sum")
                            if next_sum_check is None:
                                _LOGGER.debug(
                                    "ADJUST: No missing energy estimate for %s - "
                                    "next entry has no sum value",
                                    entity_id,
                                )
                            elif next_sum_check <= current_sum:
                                _LOGGER.debug(
                                    "ADJUST: No missing energy estimate for %s - "
