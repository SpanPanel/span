    ...
    try:
        with session_scope(hass=hass) as session:
            ...
            for reset_time in reset_timestamps:
                ...
                # The hourly entry containing reset_ts starts at the beginning of that hour
                # We need to target the hour-aligned timestamp, not the 5-minute timestamp
                hour_start_ts = (reset_ts // 3600) * 3600  # Floor to hour boundary
                # Use 2-minute window around the hour start to catch the hourly entry
                long_term_window_start = hour_start_ts - 120
                long_term_window_end = hour_start_ts + 120

                # Log in local time for user visibility, but use UTC for database operations
                local_time = dt_util.as_local(reset_time)
                hour_start_time = dt_util.utc_from_timestamp(hour_start_ts)
                _LOGGER.info(
                    "Processing reset at %s (local: %s). "
                    "Short-term window: %.1f to %.1f. "
                    "Long-term (hourly) window around %s: %.1f to %.1f",
                    reset_time.isoformat(),
                    local_time.isoformat(),
                    short_term_window_start,
                    short_term_window_end,
                    hour_start_time.isoformat(),
