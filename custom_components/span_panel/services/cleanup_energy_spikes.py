...
async def cleanup_energy_spikes(
    ...
    if not dry_run:
        ...
        try:
            # Adjust all sensors with negative spikes (delta < 0)
            # "Zero out" means adding an adjustment at the spike point to restore continuity
            # Only adjust sensors that actually have negative deltas (actual spikes)
            sensors_with_spikes = {
                detail["entity_id"]
            ...
            _LOGGER.warning(
                "ADJUST: Adjusting %d sensors with negative spikes (out of %d total sensors)",
        ...
        for entity_id in span_energy_sensors:
            ...
            for stat_entry in sensor_stats:
                ...
                for reset_time in reset_timestamps:
                    ...
                    # Statistics are hourly, so we match the hour-aligned reset timestamp
                    if abs((entry_time - reset_time).total_seconds()) <= 120:
                        ...
                        if idx > 0:
                            ...
                            if prev_val is not None:
                                ...
                                # Only include entries with negative deltas (actual spikes)
                                # Positive deltas are normal increases, not spikes
                                if delta < 0:
                                    spikes.append(
                                        {
                ...
                if time_diff <= 120:
                    ...
                    if i > 0 and current_sum is not None:
                        ...
                        if prev_sum is not None and prev_sum > current_sum:
                            ...
                            _LOGGER.warning(
                                "ADJUST: Found spike in %s: "
                                "prev_entry[%s]=%.2f, current_entry[%s]=%.2f, "
                                "adjustment=+%.2f",
                                entity_id,
                                prev_time.isoformat(),
                                prev_sum,
