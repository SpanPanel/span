...
def _delete_statistics_entries_sync(
    ...
    # This handles commit, rollback, and close automatically
    # Note: We use hass parameter to explicitly create a new session.
    # Do NOT use session=recorder.get_session() as get_session() returns
    # a scoped_session factory, not an active Session instance.
    try:
        with recorder.session_scope(hass=hass) as session:
            ...
            for reset_time in reset_timestamps:
                ...
                # Match the preview logic: abs(difference) <= 300 seconds
                window_start = reset_ts - 300  # 5 minutes before
                window_end = reset_ts + 300  # 5 minutes after

                _LOGGER.info(
                    "Processing reset at %s (ts=%.1f), window: %.1f to %.1f",
                    reset_time.isoformat(),
                    reset_ts,
                    window_start,
                    window_end,
                ...
                for entity_id in span_energy_sensors:
                    ...
                    _LOGGER.debug(
                        "Found %d short-term and %d long-term entries for %s at %s",
                        short_term_count,
                        long_term_count,
                        entity_id,
                        reset_time.isoformat(),
                    ...
                    if total_deleted > 0:
                        ...
                        _LOGGER.warning(
                            "No entries deleted for %s at %s despite finding %d short-term and %d long-term entries (window: %.1f to %.1f)",
                            entity_id,
                            reset_time.isoformat(),
                            short_term_count,
                            long_term_count,
