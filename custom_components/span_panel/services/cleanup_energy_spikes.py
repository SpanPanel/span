...
async def cleanup_energy_spikes(
    ...
    if start_time_utc >= end_time_utc:
        _LOGGER.error(
            "Invalid time range: start_time (%s) must be before end_time (%s)", start_time, end_time
        )
        return {
            "dry_run": dry_run,
            "config_entry_id": config_entry_id,
    ...
    try:
        reset_timestamps = await _find_reset_timestamps(
            hass, main_meter_entity, start_time_utc, end_time_utc
        )
    ...
    details = await _collect_spike_details(
        hass, entry_sensors, reset_timestamps, start_time_utc, end_time_utc
    )
...
async def _async_delay(hass: HomeAssistant, delay_seconds: float) -> None:
    ...
    This is the proper way to add delays in Home Assistant instead of
    using asyncio.sleep(), as it properly integrates with the event loop.
