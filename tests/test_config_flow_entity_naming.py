...
def mock_config_entry():
    ...
    # Create a simple object that doesn't inherit from Home Assistant classes
...
def mock_frame_helper():
    """Mock the frame helper to avoid 'Frame helper not set up' errors."""
...
class TestEntityNamingOptions:
    ...
    def create_flow(self, mock_config_entry, mock_hass):
        """Create an OptionsFlowHandler instance without triggering frame helper."""
        with (
            patch.object(OptionsFlowHandler, "__init__", return_value=None),
            patch.object(OptionsFlowHandler, "config_entry", mock_config_entry),
        ):
