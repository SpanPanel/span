"""Test SPAN Panel migration from v1.0.4 to v1.2.0.

This test validates the complete migration process:
1. Loads real v1.0.4 entity registry data (copied to temp location)
...
async def test_migration_1_0_4_to_1_2_0():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        source_registry_dir = Path(__file__).parent / "migration_storage" / "1_0_4"
        ...
        for entity in span_entities:
            ...
            if any(key in unique_id for key in ["mainMeterEnergy", "feedthroughEnergy", "feedthroughPowerW", "instantGridPowerW"]):
                ...
                if len(parts) >= 4 and len(parts[2]) == 32 and all(c in '0123456789abcdef' for c in parts[2]):
                    ...
                    # v1.0.4 pattern: span_nj-2316-005k6_{circuit_id}_instantPowerW
                    circuit_entities.append(entity)
                else:
...
@pytest.mark.asyncio
async def test_actual_migration_process_1_0_4_to_1_2_0():
    """Test the actual migration process by running the migration function with real registry data."""
    
    print("🧪 SPAN Panel Actual Migration Test: v1.0.4 → v1.2.0")
    print("=" * 60)
...
@pytest.mark.asyncio
async def test_circuit_power_sensor_filtering_fix():
    """Test that the circuit power sensor filtering bug is fixed.
    
    This test validates that circuit power sensors are correctly identified
    in the power sensor breakdown debug output.
    """
    
    print("🧪 Circuit Power Sensor Filtering Fix Test")
    print("=" * 60)
    
    # Test the fixed filtering logic directly
    import re
    
    # Simulate sensor keys from a real migration
    test_sensor_configs = {
        # Panel power sensors
        "span_nj-2316-005k6_current_power": {"template": "panel_sensor"},
        "span_nj-2316-005k6_feed_through_power": {"template": "panel_sensor"},
        
        # Circuit power sensors (these were being missed before the fix)
        "span_nj-2316-005k6_0dad2f16cd514812ae1807b0457d473e_power": {"template": "circuit_power"},
        "span_nj-2316-005k6_11a47a0f69d54e12b7200f730c2ffda1_power": {"template": "circuit_power"},
        "span_nj-2316-005k6_12ce227695cd44338864b0ef2ec4168b_power": {"template": "circuit_power"},
        
        # Circuit energy sensors
        "span_nj-2316-005k6_0dad2f16cd514812ae1807b0457d473e_energy_produced": {"template": "circuit_energy_produced"},
        "span_nj-2316-005k6_0dad2f16cd514812ae1807b0457d473e_energy_consumed": {"template": "circuit_energy_consumed"},
        
        # Panel energy sensors
        "span_nj-2316-005k6_main_meter_produced_energy": {"template": "panel_energy_produced"},
        "span_nj-2316-005k6_main_meter_consumed_energy": {"template": "panel_energy_consumed"},
        "span_nj-2316-005k6_feed_through_produced_energy": {"template": "panel_energy_produced"},
        "span_nj-2316-005k6_feed_through_consumed_energy": {"template": "panel_energy_consumed"},
    }
    
    print(f"📊 Testing with {len(test_sensor_configs)} sensor configurations")
    
    # Apply the FIXED filtering logic
    power_sensors = [key for key in test_sensor_configs if "power" in key.lower()]
    
    # OLD BROKEN CODE (would return 0):
    # circuit_power_sensors_old = [key for key in power_sensors if "circuit" in key.lower()]
    ...
    print(f"📊 Power Sensor Filtering Results:")
    print(f"   Total power sensors: {len(power_sensors)}")
    print(f"   Circuit power sensors (FIXED): {len(circuit_power_sensors)}")
    print(f"   Circuit power sensors found: {circuit_power_sensors}")
    
    # Validate the fix
    expected_circuit_power_count = 3  # We have 3 circuit power sensors in test data
    assert len(circuit_power_sensors) == expected_circuit_power_count, f"Expected {expected_circuit_power_count} circuit power sensors, got {len(circuit_power_sensors)}"
    
    # Validate that all circuit power sensors are found
    expected_circuit_power_sensors = [
        "span_nj-2316-005k6_0dad2f16cd514812ae1807b0457d473e_power",
        "span_nj-2316-005k6_11a47a0f69d54e12b7200f730c2ffda1_power", 
        "span_nj-2316-005k6_12ce227695cd44338864b0ef2ec4168b_power"
    ]
    
    for expected_sensor in expected_circuit_power_sensors:
        assert expected_sensor in circuit_power_sensors, f"Expected circuit power sensor {expected_sensor} not found"
    
    # Validate that panel power sensors are NOT incorrectly identified as circuit power sensors
    panel_power_sensors = [key for key in power_sensors if not re.search(uuid_power_pattern, key)]
    expected_panel_power_count = 2
    assert len(panel_power_sensors) == expected_panel_power_count, f"Expected {expected_panel_power_count} panel power sensors, got {len(panel_power_sensors)}"
    
    print(f"✅ Circuit power sensor filtering fix VERIFIED!")
    print(f"   Before fix: 0 circuit power sensors would be found")
    print(f"   After fix: {len(circuit_power_sensors)} circuit power sensors correctly identified")
    print(f"   Panel power sensors: {len(panel_power_sensors)} correctly identified")
    
    print(f"\n" + "=" * 60)
    print(f"CIRCUIT POWER SENSOR FILTERING TEST PASSED!")
    print(f"   The regex pattern correctly identifies circuit power sensors")
    print(f"   The bug that caused missing circuit power sensors is FIXED!")


@pytest.mark.asyncio
async def test_solar_sensor_migration_fix():
    """Test that solar sensor migration is fixed.
    
    This test validates that solar sensors are correctly migrated
    from the current format to the new format.
    """
    
    print("🧪 Solar Sensor Migration Fix Test")
    print("=" * 60)
    
    # Import the migration function
    from custom_components.span_panel.migration import _compute_normalized_unique_id
    
    # Test solar sensor unique ID normalization
    test_cases = [
        # Current format (what's in the registry)
        ("span_nj-2316-005k6_solar_current_power", "span_nj-2316-005k6_solar_current_power"),
        ("span_nj-2316-005k6_solar_produced_energy", "span_nj-2316-005k6_solar_produced_energy"),
        ("span_nj-2316-005k6_solar_consumed_energy", "span_nj-2316-005k6_solar_consumed_energy"),
        
        # Old synthetic format (if any exist)
        ("span_nj-2316-005k6_synthetic_30_32_solar_inverter_instant_power", "span_nj-2316-005k6_solar_current_power"),
        ("span_nj-2316-005k6_synthetic_30_32_solar_inverter_energy_produced", "span_nj-2316-005k6_solar_produced_energy"),
        ("span_nj-2316-005k6_synthetic_30_32_solar_inverter_energy_consumed", "span_nj-2316-005k6_solar_consumed_energy"),
    ]
    
    print(f"📊 Testing {len(test_cases)} solar sensor migration cases")
    
    for old_unique_id, expected_new_unique_id in test_cases:
        result = _compute_normalized_unique_id(old_unique_id)
        print(f"   {old_unique_id} → {result}")
        
        if result != expected_new_unique_id:
            pytest.fail(f"Solar sensor migration failed: {old_unique_id} → {result} (expected {expected_new_unique_id})")
    
    print(f"✅ All solar sensor migrations work correctly!")
    
    # Test that the migration produces the expected unique IDs that the solar system looks for
    expected_solar_unique_ids = [
        "span_nj-2316-005k6_solar_current_power",
        "span_nj-2316-005k6_solar_produced_energy", 
        "span_nj-2316-005k6_solar_consumed_energy"
    ]
    
    print(f"📊 Expected solar unique IDs for migration lookup:")
    for unique_id in expected_solar_unique_ids:
        print(f"   {unique_id}")
    
    print(f"\n" + "=" * 60)
    print(f"SOLAR SENSOR MIGRATION TEST PASSED!")
    print(f"   Solar sensors will now be migrated correctly in Phase 1")
    print(f"   The solar sensor lookup errors in Phase 3 will be FIXED!")
