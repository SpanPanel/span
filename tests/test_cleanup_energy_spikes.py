...
class TestCleanupEnergySpikes:
    ...
    async def test_spike_adjusted_when_not_dry_run(
        ...
        assert len(result["reset_timestamps"]) == 1
        # Only main meter sensors are adjusted (consumed_energy and produced_energy)
        # Kitchen sensor is not adjusted as it's not a main meter sensor
        assert result["sensors_adjusted"] == 2  # Only 2 main meter sensors adjusted
        assert "error" not in result  # No error should be present
        # Verify async_adjust_statistics was called for each main meter sensor
        assert mock_recorder.async_adjust_statistics.call_count == 2
