...
class TestMonitoringOptionsSchema:
    ...
    def test_settings_schema_does_not_include_toggle(self):
        """Settings schema does not include the monitoring toggle (it's in the prior step)."""
        entry = _make_entry()
        schema = build_monitoring_settings_schema(entry)
        keys = [str(k) for k in schema.schema]
        assert ENABLE_CURRENT_MONITORING not in keys
...
class TestMonitoringOptionsDefaults:
    ...
    def test_defaults_threshold_values(self):
        ...
        entry = _make_entry()
        defaults = get_monitoring_settings_defaults(entry)
