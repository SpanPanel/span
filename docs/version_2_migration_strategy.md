...
This migration strategy prioritizes user experience and data preservation while enabling the transition to the new synthetic sensor architecture. By generating
YAML configurations from existing installations rather than attempting to normalize unique IDs, we ensure a smooth upgrade process that maintains all existing
functionality while unlocking new synthetic sensor capabilities.
