...
When the SPAN panel undergoes a firmware update or reset, it may report incorrect energy values in otherwise TOTAL_INCREASING SENSORS, losing some range of
...
is to adjust Home Assistant statistics for SPAN sensors.
...
1. **Backup the system** - Create a backup of your Home Assistant instance before making any changes. This process is irreversable.
...
6. First run with `dry_run: true` to preview what will be adjusted (important, save the JSON in case you need to undo)
7. Review the persistent notification showing affected timestamps
8. Run again with `dry_run: false` to adjust the problematic entries
9. Fine tune using the statistics adjustments if necessary

**Note:** The integration monitors for firmware resets and will display a notification when one is detected.
